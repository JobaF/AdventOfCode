...
for (let i = 0; i < input.length; i++) {
  ...
  if (x1 == x2) {
    ...
    //Möglichkeit 1: oben links -> unten rechts
    const lengthBetweenPoints = Math.abs(y2 - y1)

    if (x1 < x2 && y1 < y2) {
      for (let j = 0; j <= lengthBetweenPoints; j++) {
        hydrothermalVents[x1 + j][y1 + j] += 1
      }
    }
    //Möglichkeit 2: oben rechts -> unten links
    else if (x1 > x2 && y1 < y2) {
      for (let j = 0; j <= lengthBetweenPoints; j++) {
        hydrothermalVents[x1 - j][y1 + j] += 1
      }
    }
    //Möglichkeit 3: unten rechts -> oben links
    else if (x1 > x2 && y1 > y2) {
      for (let j = 0; j <= lengthBetweenPoints; j++) {
        hydrothermalVents[x1 - j][y1 - j] += 1
      }
    }
    //Möglichkeit 4: unten links -> oben rechts
    else if (x1 < x2 && y1 > y2) {
      for (let j = 0; j <= lengthBetweenPoints; j++) {
        hydrothermalVents[x1 + j][y1 - j] += 1
      }
